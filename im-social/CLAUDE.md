...
- 双向好友关系模型（双向写入/删除/状态同步，保证数据对称性）
- 好友申请审批流水线（申请 → 审批 → Feign 创建会话 → 双向写库 → Redis 缓存双写）
- 确定性 conversationId 生成算法（minId_maxId 拼接，无中心化 ID 分配的分布式唯一性）
- Redis Set 离线增量过滤（基于离线时间戳过滤新增好友变更）
- 在线状态实时感知（Redis Set SISMEMBER 判断，返回 isOnline 标志）
- Feign 跨服务编排（调用 im-chat 创建会话/邀请成员/更新群信息）

> 我设计并实现了一套基于双向操作模型的好友关系微服务，通过"申请 → 审批 → Feign 远程创建会话 → 双向写入 friend_relation → Redis 缓存双写"的完整流水线覆盖好友关系全生命周期，通过确定性 ID 生成（minId_maxId 拼接）保证会话唯一性而无需中心化发号器，通过 Redis Set 实现在线状态感知与离线增量过滤，本质上是一个**通过 Feign 编排跨服务事务的社交关系中心**。
